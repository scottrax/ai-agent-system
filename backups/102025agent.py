...
class AIAgent:
    def __init__(self, api_key, log_file=None):
        ...
        # Store both clients for fallback
        self.anthropic_client = None
        self.ollama_client = None
        
        # Initialize Anthropic client if key available
        if self.anthropic_key:
            try:
                self.anthropic_client = Anthropic(api_key=self.anthropic_key)
                logger.info("Anthropic client initialized (available for fallback)")
        ...
        if self.provider == "ollama":
            if not HAS_OPENAI:
                if self.anthropic_client:
                    logger.warning("OpenAI library not found for Ollama. Falling back to Anthropic")
